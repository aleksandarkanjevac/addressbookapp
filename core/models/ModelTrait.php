...
trait ModelTrait
{
    ...
    */
     ...
     * @param array $data
     */
    public static function deleteByAttributes(array $data) {
                ...
                try {
                    ...
                    $stmt = $conn->prepare('DELETE FROM ' . self::table() . ' WHERE ' . implode(' AND ', $datas));
                    foreach ($data as $dat => $val) {
                        $stmt->bindValue(":{$dat}", $val);
                    }
                    $stmt->execute();
                } catch (Exception $ex) {
                ...
                /**
     * update data in database table based on value which will be set and array,
     * if array not found throw Exception 
     * @param array $data
     * @param array $attrs
     * @throws \Exception
     */
    public static function updateByAttributes(array $data, array $attrs) {
        if (count($attrs) == 0) {
            throw new \Exception('Please send some datas');
        }
        ...
        try {
            ...
            $stmt = $conn->prepare('UPDATE ' . self::table() . ' SET ' . implode('=', $datas) . ' WHERE ' . implode(' AND ', $where));
     ...
     public static function search($query) {
                ...
                $stmt = $conn->prepare('SELECT * FROM ' . self::table() . ' WHERE first_name LIKE :fname OR last_name LIKE :lname');
                $stmt->bindValue(':fname', "%{$query}%");
                $stmt->bindValue(':lname', "%{$query}%");
                $stmt->execute();
                $stmt->setFetchMode(\PDO::FETCH_ASSOC);
